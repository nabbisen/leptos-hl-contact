...
| FR-ABUSE-03 | `form_token::an_old_token_is_expired`, `form_token::a_far_future_token_is_rejected`, `form_token::a_tampered_signature_is_a_bad_signature`, `form_token::a_token_at_exactly_its_ttl_is_valid_one_second_later_it_is_expired`, `form_token::a_token_at_exactly_the_future_skew_limit_is_accepted_one_second_beyond_is_refused` | `form_token::a_missing_malformed_or_expired_token_is_rejected` | — | constant-time comparison: review (`form_token.rs`, `constant_time_eq`); see NFR-SEC-03 |
...
| FR-CFG-02 | `axum_helpers::delivery_context_fn_is_clone` | `routing::context_in_the_one_closure_reaches_submit_contact` and every `delivery::` test (the server harness calls `provide_contact_delivery` itself, RFC 020 D1 row 4) | — | the documentation of each context value: documentation |
...
| FR-CFG-04 | `form_token::debug_redacts_the_secret`, `challenge::http::debug_redacts_the_secret`, `config::redirect_debug_does_not_expose_the_executor`, `delivery::smtp::debug_redacts_the_password`, `delivery::resend::debug_redacts_the_key`, `form_token::form_token_issuer_debug_is_the_fixed_placeholder`, `challenge::a_challenge_context_debug_names_the_struct_and_hides_the_verifier`, `filter::filter_chain_debug_lists_each_filters_name_in_order` | — | — | — |
