...
|----|------|----------|------|----------|
| P-35 | An opt-in check that an address's domain can receive mail at all: MX with the RFC 5321 §5.1 address-record fallback, null MX refused, every failure of ours accepted with a `warn`; the lookup goes over the shared HTTPS transport so it works natively **and** on a Worker, which has no UDP | **High** | [RFC 018](./rfcs/accepted/018-email-domain-check.md) (accepted 2026-09-22) — spike approved, DoH stands; handoffs 02 and 03 conditionally approved 2026-09-22 (`b8d0d1c`, `691a1e3`: the lookup, the decision table, the pipeline step and the visitor's message), r2 for two small robustness conditions; handoff 04 approved (`b73eb7a`: documentation, privacy, traceability, and the live tests run) | the owner's original question, 2026-09-13: a syntactically valid address that cannot receive mail wastes the site owner's reply |
