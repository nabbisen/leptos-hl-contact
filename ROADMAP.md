...
| P-45 | A flaky browser test: `the_submitted_body_carries_the_site_fields` asserts on a request body the stub records asynchronously, after a single settle; it failed once in five local runs on code untouched since 0.8.0.  Wait for the record instead of assuming it, for every test that asserts on captured bodies.  A gate that fails at random teaches everyone to re-run it | **Medium** — a release gate | handoff (tests only), proposed 2026-09-22 | architect's own verification run of `d92d522` |
