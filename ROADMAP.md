...
|----|------|----------|------|----------|
| P-35 | An opt-in check that an address's domain can receive mail at all: MX with the RFC 5321 §5.1 address-record fallback, null MX refused, every failure of ours accepted with a `warn`; the lookup goes over the shared HTTPS transport so it works natively **and** on a Worker, which has no UDP | **High** | [RFC 018](./rfcs/accepted/018-email-domain-check.md) (accepted 2026-09-22) — spike approved 2026-09-22, DoH stands (a GET is needed on both targets; NODATA is a third refusal shape); handoffs 02–04 written | the owner's original question, 2026-09-13: a syntactically valid address that cannot receive mail wastes the site owner's reply |
