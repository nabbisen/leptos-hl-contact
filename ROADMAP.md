...
| ID | Item | Priority | Kind | Evidence |
|----|------|----------|------|----------|
| P-10 | Form subtree rebuilt whenever the action value changes.  **Corrected 2026-09-12 by the hydrated example:** tachys rebuilds in place, so typed input is *not* lost; what the rebuild does break is the hidden token attribute (P-11).  Build-once remains the design (RFC 002 D1) because attribute rebuilds are the mechanism of P-11 | **High** | [RFC 002](./rfcs/accepted/002-form-state-model.md) | verified in the browser (handoff 002-01) |
